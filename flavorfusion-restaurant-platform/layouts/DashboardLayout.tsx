    ...
    return (
        ...
        `}>
            <div className="flex items-center justify-between h-16 px-4 border-b border-gray-700 dark:border-gray-800 flex-shrink-0">
                <Link to="/" className={`flex items-center gap-3 text-2xl font-bold ${isCollapsed ? 'lg:hidden' : ''}`}>
                    <FlavorFusionLogo />
                    <span>{settings.restaurantName}</span>
                </Link>
                <Link to="/" className={`hidden ${isCollapsed ? 'lg:flex items-center justify-center' : ''}`}>
                    <FlavorFusionLogo />
                </Link>
            ...
            <nav className="flex-grow p-4 space-y-2 overflow-y-auto">
                {links.map((link) => {
                    const isActive = location.pathname.startsWith(link.to) && (link.to !== '/dashboard' || location.pathname === '/dashboard' || location.pathname === '/dashboard/pos');
                    return (
                        <Link
                            key={link.to}
                            to={link.to}
        ...
        <header className="bg-white dark:bg-gray-800 shadow-sm z-10">
          <div className="flex items-center justify-between h-16 px-6">
            ...
            <div className="flex items-center gap-4">
                ...
                <Link to={getProfileLink(permissions)}>
