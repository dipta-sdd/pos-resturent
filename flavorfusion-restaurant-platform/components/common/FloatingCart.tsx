                ...
                <div className={`absolute top-0 right-0 h-full w-full max-w-md bg-white dark:bg-gray-800 shadow-xl flex flex-col transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                        ...
                        <div className="flex-grow overflow-y-auto p-6 space-y-4">
                            {cartItems.map(item => (
                                ...
                                <div key={`${item.menuItem.id}-${item.variant.id}`} className="flex items-center gap-4">
                                    <img src={item.menuItem.image_url} alt={item.menuItem.name} className="w-20 h-20 rounded-lg object-cover" />
                    ...
                    {cartCount > 0 && (
                        <div className="p-6 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <Link to="/checkout" onClick={handleCheckout} className="w-full bg-orange-500 text-white font-bold py-3.5 px-6 rounded-xl hover:bg-orange-600 transition-colors text-lg flex items-center justify-center gap-2 shadow-lg shadow-orange-500/20">
                                Go to Checkout
                            </Link>
                        </div>
                    )}
