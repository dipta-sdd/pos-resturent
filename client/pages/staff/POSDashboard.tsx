    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-4">
            <Link to="/pos/order/new-takeaway" className="bg-blue-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-600">New Takeaway</Link>
            <Link to="/pos/order/new-dinein" className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600">New Dine-In</Link>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {activeOrders.map(order => (
                 <Link to={`/pos/order/${order.id}`} key={order.id} className={`bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md border-l-4 ${getStatusColor(order.status)} hover:shadow-lg transition-shadow`}>
