...
const OrderSummary: React.FC<{ tipAmount: number; onTipChange: (type: '10' | '15' | '20' | 'custom', value: number) => void; customTip: string; onCustomTipChange: (value: string) => void; tipSelection: string | null }> = ({ tipAmount, onTipChange, customTip, onCustomTipChange, tipSelection }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200/80 dark:border-gray-700/60 h-fit lg:sticky top-28">
            ...
            <button className="w-full mt-6 bg-orange-500 text-white font-bold py-3.5 px-6 rounded-xl hover:bg-orange-600 transition-colors text-lg flex items-center justify-center gap-2 shadow-lg shadow-orange-500/20 hover:shadow-xl hover:shadow-orange-500/30">
...
const CheckoutPage: React.FC = () => {
    ...
    const handleSaveNewAddress = (addressData: { label: string, full_address: string, city: string | null, zip_code: string | null }) => {
        ...
        // Mock adding a new address since there's no API endpoint for it
        const newAddress: Address = {
            ...
            is_default: addresses.length === 0, // Make it default if it's the first one
        };
                                ...
                                <div className="space-y-4">
                                    {addresses.map(address => (
                                        <label key={address.id} className={`cursor-pointer p-5 rounded-xl border-2 transition-all duration-200 flex items-center justify-between ${selectedAddressId === address.id ? 'border-orange-500 bg-orange-500/5' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                                            ...
                                            <button className="text-gray-400 hover:text-orange-500"><Pencil size={18}/></button>
                                        </label>
                                    ))}
                                    {isAddingAddress ? (
                                        <AddressEditor 
